...
// Ruta para cancelar un turno (Validado)
app.delete('/cancelar-turno/:id_turno', autenticarUsuario, (req, res) => {
  const idTurnoParam = req.params.id_turno.trim();       
  const idCuentaUser = req.usuario.id_cuenta.trim();     

  // Busca comparando strings
  const turno = turnos.find(t =>
    String(t.id_turno).trim() === idTurnoParam &&
    String(t.id_cuenta).trim()   === idCuentaUser
  );

  if (!turno) {
    console.log(
      'DEBUG ➞',
      't.id_turno=', t?.id_turno, typeof t?.id_turno,
      'param=', idTurnoParam, typeof idTurnoParam,
      't.id_cuenta=', t?.id_cuenta, typeof t?.id_cuenta,
      'usuario.id_cuenta=', idCuentaUser, typeof idCuentaUser
    );
    return res
      .status(404)
      .json({ mensaje: 'Turno no encontrado o no pertenece a esta cuenta.' });
  }
  ...
  return res
    .status(200)
    .json({ mensaje: 'Turno cancelado correctamente.', turno });
});
